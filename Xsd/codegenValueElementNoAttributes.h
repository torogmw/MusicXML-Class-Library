...
namespace xsd
{
    inline bool codegenIsValueElementNoAttributes( MsItemElementPtr& element )
    {
    ...
    inline void codegenValueElementNoAttributes( MsItemElementPtr& e, bool append = false, bool setImplemented = false )
    {
        std::stringstream h;
        ...
        XpItemPtr xp = e->getXpItem();
        auto xpchildrencount = xp->getChildren().size();
        XpItemPtr firstchild;
        XpPropertyPtr typeproperty;
        std::string type_name;
        MsItemPtr simpletypemsitem;
        MsItemSimpleTypePtr simpletypeXXX;
            ...
            )
        {
            if ( xp->getProperties().size() >= 2 )
            {
                ...
                {
                    type_name = typeproperty->getValue();
                    simpletypemsitem = findItemByNameAndKind( type_name, MsItemKind::simpleType, e );
                    if ( simpletypemsitem )
                    {
                        simpletypeXXX = std::make_shared<MsItemSimpleType>( *simpletypemsitem );
                    }
                }
            }
        }
        ...
        h << e->getXml() << end();
        simpletypeXXX->getXpItem()->stream( h, 0 );
        //h << end();
        simpletypemsitem->getXpItem()->stream( h, 0 );
        //h << end();
        h << "*/" << end();
//        /* H ATTRIBUTES STRUCT */
...
//        h << "};" << end(2);
        ...
        h << tab(1) << e->getCppName() << "( const types::" << simpletypeXXX->getCppName() << "& value );" << end();
        ...
        h << tab(1) << "types::" << simpletypeXXX->getCppName() << " getValue() const;" << end();
        h << tab(1) << "void setValue( const types::" << simpletypeXXX->getCppName() << "& value );" << end();
        h << "private:" << end();
        h << tab(1) << "types::" << simpletypeXXX->getCppName() << " myValue;" << end();
...
//        cpp << "}" << end(2);
        ...
        cpp << ":myValue(";
        if ( simpletypeXXX->getMsItemSimpleTypeKind() == MsItemSimpleTypeKind::enumeration )
        {
            cpp << " types::" << simpletypeXXX->getCppName() << ":: ";
        }
        cpp << ")" << end();
        //cpp << ",myAttributes( std::make_shared<" << attStructName << ">() )" << end();
        cpp << "{}" << end();
        
        cpp << e->getCppName() << "::" << e->getCppName() << "( const types::" << simpletypeXXX->getCppName() << "& value )" << end();
...
//        cpp << "}" << end();
        ...
        cpp << "types::" << simpletypeXXX->getCppName() << " " << e->getCppName() << "::getValue() const" << end();
        cpp << "{" << end();
        cpp << "return myValue;" << end();
        cpp << "}" << end();
        
        cpp << "void " << e->getCppName() << "::setValue( const types::" << simpletypeXXX->getCppName() << "& value )" << end();
        cpp << "{" << end();
        cpp << "myValue = value;" << end();
        cpp << "}" << end();
        ...
        test << tab(1) << simpletypeXXX->getCppName() << " value1;" << end();
        test << tab(1) << simpletypeXXX->getCppName() << " value2;" << end();
