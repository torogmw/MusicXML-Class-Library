...
namespace xsd
{
    inline void codegenAttributesStructCpp( std::ostream& cpp, const std::string& attStructName, const MsItemElementPtr& e, bool skipIfDuplicate );
    ...
    inline void codegenAttributesStructCpp( std::ostream& cpp, const std::string& attStructName, const MsItemElementPtr& e, bool skipIfDuplicate )
    {
        if ( attStructName != "AttributeInterface" )
        {
            ...
            {
                cpp << "/* skipped " << e->getID() << " " << attStructName << " */" << end(2);
            }
        }
    }
}
